...
with open('languages.txt') as f:
    lines = [line.strip().replace('- ', '').replace(' (beta)', '').replace(' (experimental)', '').replace(' (deprecated)', '') for line in f]

lines = list(filter(None, lines))
language_list = lines[lines.index("CLIENT generators:")+1: lines.index("SERVER generators:")]
