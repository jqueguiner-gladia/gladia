(ns fast-api.api.text-text-keyword-extraction
...
(defn-spec apply-text-text-keyword-extraction-post-with-http-info any?
  "Apply model for the keyword-extraction task for a given models"
  ([] (apply-text-text-keyword-extraction-post-with-http-info nil))
  ([{:keys [text model]} (s/map-of keyword? any?)]
   (call-api "/text/text/keyword-extraction/" :post
             {:path-params   {}
              :header-params {}
...
(defn-spec apply-text-text-keyword-extraction-post any?
  "Apply model for the keyword-extraction task for a given models"
  ([] (apply-text-text-keyword-extraction-post nil))
  ([optional-params any?]
   (let [res (:data (apply-text-text-keyword-extraction-post-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode any? res st/string-transformer)
        res))))


(defn-spec get-versions-text-text-keyword-extraction-get-with-http-info any?
  "Get list of models available for keyword-extraction"
  []
  (call-api "/text/text/keyword-extraction/" :get
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec get-versions-text-text-keyword-extraction-get any?
  "Get list of models available for keyword-extraction"
  []
  (let [res (:data (get-versions-text-text-keyword-extraction-get-with-http-info))]
    (if (:decode-models *api-context*)
       (st/decode any? res st/string-transformer)
       res)))
