(ns fast-api.api.image-image-background-removal
...
(defn-spec apply-image-image-background-removal-post-with-http-info any?
  "Apply model for the background-removal task for a given models"
  ([^File image any?, ] (apply-image-image-background-removal-post-with-http-info image nil))
  ([^File image any?, {:keys [model]} (s/map-of keyword? any?)]
   (check-required-params image)
   (call-api "/image/image/background-removal/" :post
             {:path-params   {}
              :header-params {}
...
(defn-spec apply-image-image-background-removal-post any?
  "Apply model for the background-removal task for a given models"
  ([^File image any?, ] (apply-image-image-background-removal-post image nil))
  ([^File image any?, optional-params any?]
   (let [res (:data (apply-image-image-background-removal-post-with-http-info image optional-params))]
     (if (:decode-models *api-context*)
        (st/decode any? res st/string-transformer)
        res))))


(defn-spec get-versions-image-image-background-removal-get-with-http-info any?
  "Get list of models available for background-removal"
  []
  (call-api "/image/image/background-removal/" :get
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec get-versions-image-image-background-removal-get any?
  "Get list of models available for background-removal"
  []
  (let [res (:data (get-versions-image-image-background-removal-get-with-http-info))]
    (if (:decode-models *api-context*)
       (st/decode any? res st/string-transformer)
       res)))
