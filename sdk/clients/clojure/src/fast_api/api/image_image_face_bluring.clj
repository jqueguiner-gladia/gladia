(ns fast-api.api.image-image-face-bluring
...
(defn-spec apply-image-image-face-bluring-post-with-http-info any?
  "Apply model for the face-bluring task for a given models"
  ([^File image any?, ] (apply-image-image-face-bluring-post-with-http-info image nil))
  ([^File image any?, {:keys [model]} (s/map-of keyword? any?)]
   (check-required-params image)
   (call-api "/image/image/face-bluring/" :post
             {:path-params   {}
              :header-params {}
...
(defn-spec apply-image-image-face-bluring-post any?
  "Apply model for the face-bluring task for a given models"
  ([^File image any?, ] (apply-image-image-face-bluring-post image nil))
  ([^File image any?, optional-params any?]
   (let [res (:data (apply-image-image-face-bluring-post-with-http-info image optional-params))]
     (if (:decode-models *api-context*)
        (st/decode any? res st/string-transformer)
        res))))


(defn-spec get-versions-image-image-face-bluring-get-with-http-info any?
  "Get list of models available for face-bluring"
  []
  (call-api "/image/image/face-bluring/" :get
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec get-versions-image-image-face-bluring-get any?
  "Get list of models available for face-bluring"
  []
  (let [res (:data (get-versions-image-image-face-bluring-get-with-http-info))]
    (if (:decode-models *api-context*)
       (st/decode any? res st/string-transformer)
       res)))
