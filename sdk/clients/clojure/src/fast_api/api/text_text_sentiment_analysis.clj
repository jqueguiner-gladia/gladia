(ns fast-api.api.text-text-sentiment-analysis
...
(defn-spec apply-text-text-sentiment-analysis-post-with-http-info any?
  "Apply model for the sentiment-analysis task for a given models"
  ([] (apply-text-text-sentiment-analysis-post-with-http-info nil))
  ([{:keys [text model]} (s/map-of keyword? any?)]
   (call-api "/text/text/sentiment-analysis/" :post
             {:path-params   {}
              :header-params {}
...
(defn-spec apply-text-text-sentiment-analysis-post any?
  "Apply model for the sentiment-analysis task for a given models"
  ([] (apply-text-text-sentiment-analysis-post nil))
  ([optional-params any?]
   (let [res (:data (apply-text-text-sentiment-analysis-post-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode any? res st/string-transformer)
        res))))


(defn-spec get-versions-text-text-sentiment-analysis-get-with-http-info any?
  "Get list of models available for sentiment-analysis"
  []
  (call-api "/text/text/sentiment-analysis/" :get
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec get-versions-text-text-sentiment-analysis-get any?
  "Get list of models available for sentiment-analysis"
  []
  (let [res (:data (get-versions-text-text-sentiment-analysis-get-with-http-info))]
    (if (:decode-models *api-context*)
       (st/decode any? res st/string-transformer)
       res)))
