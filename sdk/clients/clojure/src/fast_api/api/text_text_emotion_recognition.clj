(ns fast-api.api.text-text-emotion-recognition
...
(defn-spec apply-text-text-emotion-recognition-post-with-http-info any?
  "Apply model for the emotion-recognition task for a given models"
  ([] (apply-text-text-emotion-recognition-post-with-http-info nil))
  ([{:keys [text model]} (s/map-of keyword? any?)]
   (call-api "/text/text/emotion-recognition/" :post
             {:path-params   {}
              :header-params {}
...
(defn-spec apply-text-text-emotion-recognition-post any?
  "Apply model for the emotion-recognition task for a given models"
  ([] (apply-text-text-emotion-recognition-post nil))
  ([optional-params any?]
   (let [res (:data (apply-text-text-emotion-recognition-post-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode any? res st/string-transformer)
        res))))


(defn-spec get-versions-text-text-emotion-recognition-get-with-http-info any?
  "Get list of models available for emotion-recognition"
  []
  (call-api "/text/text/emotion-recognition/" :get
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec get-versions-text-text-emotion-recognition-get any?
  "Get list of models available for emotion-recognition"
  []
  (let [res (:data (get-versions-text-text-emotion-recognition-get-with-http-info))]
    (if (:decode-models *api-context*)
       (st/decode any? res st/string-transformer)
       res)))
