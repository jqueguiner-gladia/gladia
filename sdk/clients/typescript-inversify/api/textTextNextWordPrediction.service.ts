...
export class TextTextNextWordPredictionService {
    ...
    public applyTextTextNextWordPredictionPost(sentence?: string, model?: 'bert-base-uncased' | 'roberta-base' | 'distilbert-base-uncased' | 'albert-base-v2', observe: any = 'body', headers: Headers = {}): Observable<any> {
        ...
        const response: Observable<HttpResponse<object>> = this.httpClient.post(`${this.basePath}/text/text/next-word-prediction/?${queryParameters.join('&')}`, headers);
        if (observe == 'body') {
     ...
     */
    public getVersionsTextTextNextWordPredictionGet(observe?: 'body', headers?: Headers): Observable<object>;
    public getVersionsTextTextNextWordPredictionGet(observe?: 'response', headers?: Headers): Observable<HttpResponse<object>>;
    public getVersionsTextTextNextWordPredictionGet(observe: any = 'body', headers: Headers = {}): Observable<any> {
        headers['Accept'] = 'application/json';

        const response: Observable<HttpResponse<object>> = this.httpClient.get(`${this.basePath}/text/text/next-word-prediction/`, headers);
        if (observe == 'body') {
               return response.map(httpResponse => <object>(httpResponse.response));
        }
        return response;
