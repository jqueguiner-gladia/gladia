...
#include <stdio.h>
#include <ctype.h>
#include "ImageImageColorizationAPI.h"
...
ImageImageColorizationAPI_applyImageImageColorizationPost(apiClient_t *apiClient ,binary_t* image ,model_e model)
{
    ...
    // create the path
    long sizeOfPath = strlen("/image/image/colorization/")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/image/image/colorization/");
    ...
    cJSON *ImageImageColorizationAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    object_t *elementToReturn = object_parseFromJSON(ImageImageColorizationAPIlocalVarJSON);
    cJSON_Delete(ImageImageColorizationAPIlocalVarJSON);
