...
#include <stdio.h>
#include <ctype.h>
#include "ImageImageSuperResolutionAPI.h"
...
ImageImageSuperResolutionAPI_applyImageImageSuperResolutionPost(apiClient_t *apiClient ,binary_t* image ,char * model)
{
    ...
    // create the path
    long sizeOfPath = strlen("/image/image/super-resolution/")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/image/image/super-resolution/");
    ...
    cJSON *ImageImageSuperResolutionAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    object_t *elementToReturn = object_parseFromJSON(ImageImageSuperResolutionAPIlocalVarJSON);
    cJSON_Delete(ImageImageSuperResolutionAPIlocalVarJSON);
