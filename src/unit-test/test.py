...
def create_metadata_with_reponse(path, params, data, files, response):
    ...
    if output != "text":
        ...
        if input != "text":
            ...
            file_name = (
                f"from_{original_file_name}_{input_extension}.{output_extension}"
            )
            ovh_file_name = (
                f"output{task}{model}/examples/{file_name}"
                if files
                else f"output{task}{model}/example/{file_name}"
            )
        else:
