...
class IManager(metaclass=abc.ABCMeta):
    ...
    def __subclasshook__(cls, subclass):
        return (
            hasattr(subclass, "__MODEL_TYPE")
            and isinstance(subclass.__MODEL_TYPE, ModelType)
            and hasattr(subclass, "get_available_memory")
            and callable(subclass.get_available_memory)
            and hasattr(subclass, "load_model")
            and callable(subclass.load_model)
            and hasattr(subclass, "unload_model")
            and callable(subclass.unload_model)
            and hasattr(subclass, "model_can_be_unloaded")
            and callable(subclass.model_can_be_unloaded)
            and hasattr(subclass, "get_managed_models")
            and callable(subclass.get_managed_models)
        )
