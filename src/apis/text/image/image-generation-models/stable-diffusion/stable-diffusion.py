...
def predict(prompt="A high tech solarpunk utopia in the Amazon rainforest", 
        samples=1, 
        steps=40, 
        scale=7.5, 
        seed=396916372) -> Image:
    import torch
    ...
    pipe = StableDiffusionPipeline.from_pretrained(model_id, use_auth_token=os.getenv('HUGGINGFACE_ACCESS_TOKEN'), revision="fp16", torch_dtype=torch.float16)
    pipe = pipe.to(device)
