...
def load_model(pred_config, args, device):
    ...
    model = MODEL_CLASSES[args.model_type][1].from_pretrained(
        pretrained_model_name_or_path = "databuzzword/JointBERT-snips",
        args=args,
...
def predict(text):
    ...
    pred_config = This_config()
    pred_config.model_dir = os.path.join(os.path.dirname(__file__), "snips_model")
    pred_config.batch_size = 32
    pred_config.no_cuda = False
    pred_config.data_dir = os.path.join(os.path.dirname(__file__), "data")
    ...
    for words, slot_preds, intent_pred in zip(lines, slot_preds_list, intent_preds):
        ...
        output.append({"item": line.strip(), "intent": intent_label_lst[intent_pred]})
